...
public class AdministradorPersistenciaPrendaSinTemporada extends AdministradorPersistencia {
	...
	public void insert(Object o) {
		try {
			...
			PreparedStatement s = con.prepareStatement("insert into " + super.getDatabase() + ".dbo.Prendas (codigo, nombre) values (?, ?)");
	...
	public void update(Object o) {
		try {
			PrendaSinTemporada prenda = (PrendaSinTemporada) o;
			Connection con = Conexion.connect();
			PreparedStatement s = con.prepareStatement("update " + super.getDatabase() + ".dbo.Prendas set nombre = ? where codigo = ?");
	...
	public Collection<PrendaSinTemporada> obtenerPrendasSinTemporada() {
		...
		try {
			Connection con = Conexion.connect();
			PreparedStatement ps = con.prepareStatement("select * from " + super.getDatabase() + ".dbo.Prendas p where activo = 1 and temporada is null and not exists(select * from " + super.getDatabase() + ".dbo.Conjuntos_Prendas c where c.codigo_conjunto = p.codigo)");
