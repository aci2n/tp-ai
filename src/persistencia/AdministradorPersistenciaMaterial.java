...
public class AdministradorPersistenciaMaterial extends AdministradorPersistencia{
	...
	public void update(Object o) {
		...
		try{
			...
			Material m = (Material)o;
			con = Conexion.connect();
			PreparedStatement s = con.prepareStatement("update "+super.getDatabase()+".dbo.Materiales "+
					"set codigo = ?," +
					"set cuit = ?," +
					"set nombre =?," +
					"set cantidad =?," +
	...
	public void delete(Object o) {
		...
		try{
			...
			Material m = (Material)o;
			con = Conexion.connect();
			PreparedStatement s = con.prepareStatement("delete from "+super.getDatabase()+".dbo.Materiales where codigo = ?");
			s.setString(1, m.getCodigo());
			s.execute();
	...
	public Material buscarMaterial(String codigo){
		...
		try{
			...
			while(rs.next()){
				...
				m = new Material(cod, nom, puntoPedido, new Proveedor("jose","1"), cantidad, costo); // FALTA BUSCAR PROVEEDOR Y PASARLO AL CONSTRUCTOR
			}
			return m;
		}catch(Exception e){
